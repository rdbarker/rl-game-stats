import { numberToPercent } from "../lib/stringHelpers";

export const createMatchStats = (dataFromFetch) => {
  const findWinningTeam = () => {
    return dataFromFetch.blue.stats.core.goals >
      dataFromFetch.orange.stats.core.goals
      ? "blue"
      : "orange";
  };
  const findYourTeam = () => {
  ...
  const buildPlayerStats = (teamColor) => {
    return dataFromFetch[teamColor].players.map((player) => {
      return {
        ...
        shotPercentage: numberToPercent(
          player.stats.core.shooting_percentage,
          0
        ),
